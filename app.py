...
def draw_plate_png(assignments: dict, conditions_list: list, title: str = "Plate Layout") -> bytes:
    """
    Return PNG bytes of a print-ready A4-landscape plate layout.

    Legend strategy
    ---------------
    We use matplotlib's native fig.legend() with Rectangle patches as handles.
    This is completely immune to axes aspect-ratio distortion (the old Circle
    approach produced "crayon" ovals whenever the axes box was non-square).
    Each legend entry shows a coloured square swatch + bold condition name +
    grey well list on a second line, achieved by encoding the well list into
    the label string with a newline and a smaller-font workaround via two
    separate handle lists drawn one on top of the other.

    Simpler approach used here: one handle per condition, label = name only,
    then a separate call to annotate the axes with the well list beneath each
    entry — but that is fiddly with auto-layout.  Instead we embed both lines
    in the label with a unicode thin-space trick so they render at the same
    font size, then use a two-pass approach:
      pass 1 — invisible handles carry the name+wells label (sets bbox)
      pass 2 — visible square patches carry only the name (displayed text)

    Actually the cleanest robust approach: draw the legend with square swatches
    using Patch handles, name only in the label, then draw the well list as a
    second smaller-font annotation right below each entry using the legend's
    own coordinate system.  This is complex.

    SIMPLEST CORRECT APPROACH (used here):
    Build a two-line label string per condition.  Matplotlib legend wraps each
    label as a single text object — we cannot mix font sizes within one label.
    So we accept a uniform font size and rely on the string itself to convey
    both pieces of information:
        "ConditionName
  A1, A2, A3"
    This renders cleanly at fontsize 7.5 with ncols auto-computed from n.
    """
    ...
    # ── Auto-size figure height to accommodate legend rows ───────────────────
    # Each legend row is ~0.22 inches; ncols drives how many rows we need.
    # Cap ncols at 6 so labels stay readable.
    ncols_leg = min(n_conds, 6) if n_conds else 1
    n_leg_rows = math.ceil(n_conds / ncols_leg) if n_conds else 0
    legend_h = max(0.55, n_leg_rows * 0.30 + 0.25)   # inches
    fig_h = max(A4_H, 6.5 + legend_h)
    fig = plt.figure(figsize=(A4_W, fig_h), dpi=200)
    fig.patch.set_facecolor("white")

    # Plate axes — leave room at top for title, bottom for legend
    title_frac  = 0.45 / fig_h          # 0.45 in title band
    legend_frac = legend_h / fig_h
    margin_frac = 0.15 / fig_h
    plate_bot   = legend_frac + margin_frac
    plate_h     = 1.0 - plate_bot - title_frac - 0.02

    ax = fig.add_axes([0.07, plate_bot, 0.86, plate_h])
    ...
    for ci, col in enumerate(COLS):
        for ri, row in enumerate(ROWS):
            ...
            ax.add_patch(Circle((cx + 0.025, cy - 0.025), R,
                                 facecolor="#00000018", linewidth=0, zorder=1))
            ax.add_patch(Circle((cx, cy), R, facecolor=fc,
                                 edgecolor="#606060", linewidth=0.8, zorder=2))

    # ── Title ────────────────────────────────────────────────────────────────
    fig.text(0.5, 1.0 - (title_frac * 0.5), title,
             ha="center", va="center",
             fontsize=14, fontweight="bold", color="#1A1A1A",
             transform=fig.transFigure)

    # ── Legend — matplotlib Patch handles, two-line labels ───────────────────
    if conditions_list:
        handles, labels = [], []
        for cond in conditions_list:
            wells_str = ", ".join(cond.get("wells", []))
            # Truncate well list if very long so it fits on one line
            if len(wells_str) > 40:
                wells_str = wells_str[:37] + "…"
            label = cond["name"] + "\n" + wells_str
            handles.append(Patch(facecolor=cond["color"],
                                  edgecolor="#555555", linewidth=0.7))
            labels.append(label)

        # font size scales down slightly for many conditions
        fs = 7.5 if n_conds <= 18 else 6.5
...
def plot_kinetic(summary_df: pd.DataFrame, color_map: dict,
                 ...
                 line_styles: dict | None = None) -> bytes:
    """
    line_styles: optional dict mapping condition name -> matplotlib linestyle
                 e.g. {"Control": "-", "Drug": "--", "Blank": ":"}
    """
    ls_map = line_styles or {}
    ...
    for cond, g in summary_df.groupby("Condition", sort=False, observed=False):
        ...
        ls    = ls_map.get(str(cond), "-")
        ax.plot(x, y, label=str(cond), linewidth=2, color=color, linestyle=ls)
...
#
# Architecture: two isolated sections.
#
#  _plate_panel()   — @st.fragment  → only this section reruns when wells are
#                     selected/deselected.  The right panel (name, colour, assign)
#                     is completely unaffected, so typing a condition name never
#                     gets interrupted by a selection rerun.
#
#  _assign_panel()  — uses st.form   → name + colour inputs produce ZERO reruns
#                     while the user is typing.  A single rerun fires only when
#                     "Assign wells" is submitted.
# ─────────────────────────────────────────────────────────────────────────────
def tab_designer():
    left, right = st.columns([3, 2], gap="large")
    with left:
        _plate_panel()      # fragment — fast, isolated
    with right:
        _assign_panel()     # form-based — zero mid-type reruns


@st.fragment
def _plate_panel():
    """
    Runs as an isolated fragment — only this panel reruns on selection changes.

    Selection methods (all accumulate into st.session_state.selected_wells):
      1. Drag-box or click on the Plotly plate (Plotly on_select event)
      2. Row toggle buttons  (A-H)
      3. Column toggle buttons (1-12)
      4. Text range input  e.g. A1:C6
    Plotly selection REPLACES the current selection (matching how Plotly works),
    while the buttons and range input ADD TO / REMOVE FROM the current selection.
    A "Clear selection" button resets everything.
    """

    # ── Plate display with drag/click select ─────────────────────────────────
    fig = make_plate_figure(st.session_state.assignments,
                            st.session_state.selected_wells)

    event = st.plotly_chart(
        fig,
        on_select="rerun",          # fires a fragment-only rerun on selection
        key=f"plate_{st.session_state.chart_ver}",
        use_container_width=True,
        config={
            "displayModeBar": True,
            "modeBarButtonsToKeep": ["select2d", "lasso2d", "toImage"],
            "displaylogo": False,
        },
    )

    # Sync Plotly box/lasso selection → session state
    # Plotly replaces selection each time, so we accept the full new set.
    if event and hasattr(event, "selection") and event.selection:
        ...
        for pt in pts:
            ...
            if isinstance(cd, str) and re.match(r"^[A-H]\d{1,2}$", cd):
                new_sel.append(cd)
        if new_sel:
            # Merge with existing selection so row/col buttons still accumulate
            merged = set(st.session_state.selected_wells) | set(new_sel)
            st.session_state.selected_wells = sorted(merged, key=well_sort_key)

    sel = set(st.session_state.selected_wells)
    dirty = False
    ...
    for i, row in enumerate(ROWS):
        rw = [f"{row}{c}" for c in COLS]
        all_in = all(w in sel for w in rw)
        label = f"✓ {row}" if all_in else row
        if rb[i].button(label, key=f"rb_{row}", use_container_width=True):
            sel = sel - set(rw) if all_in else sel | set(rw)
            dirty = True

    # ── Column selector ───────────────────────────────────────────────────────
    st.markdown("**Select by column**")
    cb = st.columns(12)
    for i, col in enumerate(COLS):
        cw = [f"{r}{col}" for r in ROWS]
        all_in = all(w in sel for w in cw)
        label = f"✓{col}" if all_in else str(col)
        if cb[i].button(label, key=f"cb_{col}", use_container_width=True):
            sel = sel - set(cw) if all_in else sel | set(cw)
            dirty = True

    # ── Range text input ──────────────────────────────────────────────────────
    st.markdown("**Or type a range**")
    st.caption("Examples:  `A1`  ·  `A1, B2`  ·  `A1:A3`  ·  `A1:C6` (rectangular block)")
    tc1, tc2, tc3, tc4 = st.columns([5, 1, 1, 1])
    rtxt = tc1.text_input("rng", label_visibility="collapsed",
                           placeholder="A1:A3   or   B1:D6   or   G10, H11",
                           key="range_in")
    if tc2.button("Add", key="radd", type="primary", use_container_width=True):
        parsed = parse_well_range(rtxt)
        if parsed:
            sel |= set(parsed)
            dirty = True
        elif rtxt.strip():
            st.warning("No valid wells — check the format shown above.")
    if tc3.button("Remove", key="rrem", use_container_width=True):
        sel -= set(parse_well_range(rtxt))
        dirty = True
    if tc4.button("Clear", key="rclr", use_container_width=True):
        sel = set()
        dirty = True

    # ── Persist and rerun fragment only ──────────────────────────────────────
    if dirty:
        st.session_state.selected_wells = sorted(sel, key=well_sort_key)
        st.rerun()

    # ── Status bar ───────────────────────────────────────────────────────────
    if st.session_state.selected_wells:
        s = sorted(st.session_state.selected_wells, key=well_sort_key)
        st.success(f"**{len(s)}** well(s) selected:  {', '.join(s)}")
    else:
        st.info("Drag or click wells on the plate · or use the row/column buttons · or type a range above.")


def _assign_panel():
    """Right panel — st.form batches inputs so typing never causes a rerun."""

    st.subheader("Assign condition")

    # Show currently selected wells so the user knows what they are assigning
    sel = st.session_state.selected_wells
    if sel:
        st.caption(f"Wells queued: **{', '.join(sorted(sel, key=well_sort_key))}**")
    else:
        st.caption("No wells selected yet — use the controls on the left first.")

    # ── Assignment form ───────────────────────────────────────────────────────
    # clear_on_submit=True resets the text box and colour picker after each
    # successful assignment, ready for the next condition.
    used   = len(st.session_state.conditions_list)
    def_c  = DEFAULT_PALETTE[used % len(DEFAULT_PALETTE)]

    with st.form("assign_form", clear_on_submit=True):
        cond_name  = st.text_input("Condition name",
                                   placeholder="e.g.  E2 + CCCP + Glucose")
        cond_color = st.color_picker("Colour", value=def_c)
        submitted  = st.form_submit_button("✅  Assign wells",
                                           type="primary",
                                           use_container_width=True)

    # Handle submission OUTSIDE the form block so errors display cleanly
    if submitted:
        sel  = st.session_state.selected_wells   # re-read — fragment may have updated it
        name = cond_name.strip()
        if not sel:
            st.error("No wells are selected.  Use the row/column buttons or range input on the left.")
        elif not name:
            st.error("Please enter a condition name.")
        else:
            wells    = sorted(sel, key=well_sort_key)
            ...
            if existing:
                ...
                existing["wells"] += [w for w in wells if w not in set(existing["wells"])]
            else:
            ...
            st.session_state.selected_wells = []
            st.session_state.chart_ver += 1   # tells fragment to re-render plate
            st.success(f"✓  Assigned **{len(wells)}** well(s) to  **{name}**")

    # ── Load existing CSV ─────────────────────────────────────────────────────
    st.divider()
    with st.expander("📂  Load existing plate_layout.csv", expanded=False):
        st.caption("Upload a previously saved CSV to restore the full layout in the editor.")
        up = st.file_uploader("plate_layout.csv", type=["csv"], key="load_csv_up")
        if up and st.button("Load into editor", key="load_csv_btn", type="primary"):
            try:
                csv_bytes = up.read()
                assigned_df, cond_order, color_map = load_template_csv(csv_bytes)
                # Rebuild assignments and conditions_list from the CSV
                new_asgn  = {}
                new_clist = []
                for cond_name in cond_order:
                    rows = assigned_df[assigned_df["Condition"] == cond_name]
                    color = color_map.get(cond_name, "#808080")
                    wells = sorted(rows["Well"].tolist(), key=well_sort_key)
                    for rep, w in enumerate(wells, 1):
                        new_asgn[w] = {
                            "name": cond_name, "color": color,
                            "replicate": rep, "start_well": wells[0],
                        }
                    new_clist.append({"name": cond_name, "color": color, "wells": wells})
                st.session_state.assignments     = new_asgn
                st.session_state.conditions_list = new_clist
                st.session_state.undo_stack      = []
                st.session_state.selected_wells  = []
                st.session_state.template_csv    = csv_bytes
                st.session_state.chart_ver      += 1
                st.success(f"Loaded **{len(cond_order)}** condition(s) from CSV.")
            except Exception as e:
                st.error(f"Could not load CSV: {e}")

    # ── Legend + per-condition controls ──────────────────────────────────────
    st.divider()
    st.subheader("Conditions")
    if st.session_state.conditions_list:
        for ci, cond in enumerate(st.session_state.conditions_list):
            swatch = (
                f'<div style="width:16px;height:16px;border-radius:50%;'
                f'background:{cond["color"]};border:1px solid #888;'
                f'display:inline-block;vertical-align:middle;margin-right:6px"></div>'
            )
            with st.expander(f"{cond['name']}  —  {len(cond['wells'])} well(s)", expanded=False):
                st.markdown(
                    swatch + f" **{cond['name']}** · wells: `{', '.join(cond['wells'])}`",
                    unsafe_allow_html=True,
                )
                ea, eb = st.columns(2)

                # ── Rename ──────────────────────────────────────────────
                new_name = ea.text_input("Rename to", value=cond["name"],
                                          key=f"rename_{ci}")
                new_color = eb.color_picker("Colour", value=cond["color"],
                                             key=f"recolor_{ci}")
                if st.button("Apply rename / recolour", key=f"apply_{ci}"):
                    ...
                    if not new_name:
                        ...
                        for w in cond["wells"]:
                            if w in st.session_state.assignments:
                                st.session_state.assignments[w]["name"]  = new_name
                                st.session_state.assignments[w]["color"] = new_color
                        # Update conditions_list
                        st.session_state.conditions_list[ci]["name"]  = new_name
                        st.session_state.conditions_list[ci]["color"] = new_color
                        st.session_state.chart_ver += 1
                        st.success(f"Updated: {old_name!r} → {new_name!r}")

                st.divider()

                # ── Remove individual wells from this condition ──────────
                st.caption("Remove specific wells from this condition:")
                del_range = st.text_input(
                    "Wells to remove", placeholder="A1  or  A1:A3  or  A1, A2",
                    key=f"delrange_{ci}"
                )
                dc1, dc2 = st.columns(2)
                if dc1.button("Remove wells", key=f"delwells_{ci}"):
                    to_remove = set(parse_well_range(del_range)) if del_range.strip() else set()
                    if not to_remove:
                        st.warning("Enter at least one valid well ID.")
                    else:
                        for w in to_remove:
                            st.session_state.assignments.pop(w, None)
                        remaining = [w for w in cond["wells"] if w not in to_remove]
                        if remaining:
                            # Re-number replicates
                            for rep, w in enumerate(remaining, 1):
                                st.session_state.assignments[w]["replicate"] = rep
                            st.session_state.conditions_list[ci]["wells"] = remaining
                        else:
                            # Condition now empty — delete it entirely
                            st.session_state.conditions_list.pop(ci)
                        st.session_state.chart_ver += 1
                        st.rerun()

                # ── Delete entire condition ──────────────────────────────
                if dc2.button("🗑 Delete condition", key=f"delcond_{ci}",
                               type="secondary"):
                    for w in cond["wells"]:
                        st.session_state.assignments.pop(w, None)
                    st.session_state.conditions_list.pop(ci)
    ...
    st.divider()
    u1, u2 = st.columns(2)
    if u1.button("↩ Undo last", disabled=not st.session_state.undo_stack,
                 use_container_width=True):
    ...
    if u2.button("🗑 Clear all", use_container_width=True):
...
def tab_analyzer():
    ...
    # ── Kinetic time controls ────────────────────────────────────────────────
    time_filter_s  = None   # (t_min_s, t_max_s) hard window, or None = all
    custom_time_s  = None   # list of specific seconds to keep, or None = all in window

    if assay_choice == "kinetic":
        all_s    = sorted(merged["Time_s"].dropna().astype(int).unique())
        mins_all = [round(s / 60, 2) for s in all_s]
        t_min_d, t_max_d = float(mins_all[0]), float(mins_all[-1])

        st.markdown("#### Time-point selection")
        st.caption(f"Dataset: **{len(all_s)}** time points · "
                   f"{t_min_d} – {t_max_d} min "
                   f"(interval ≈ {round(mins_all[1]-mins_all[0],2) if len(mins_all)>1 else '?'} min)")

        time_mode = st.radio(
            "How to choose time points",
            ["All time points", "Range (start → end)", "Fixed interval", "Custom list"],
            horizontal=True,
        )

        if time_mode == "Range (start → end)":
            t_range = st.slider("Time range (min)", t_min_d, t_max_d,
                                (t_min_d, t_max_d), step=0.5)
            time_filter_s = (t_range[0] * 60, t_range[1] * 60)

        elif time_mode == "Fixed interval":
            col_iv1, col_iv2, col_iv3 = st.columns(3)
            iv_start = col_iv1.number_input("Start (min)", value=t_min_d,
                                            min_value=t_min_d, max_value=t_max_d, step=1.0)
            iv_end   = col_iv2.number_input("End (min)",   value=t_max_d,
                                            min_value=t_min_d, max_value=t_max_d, step=1.0)
            iv_step  = col_iv3.number_input("Interval (min)", value=5.0,
                                            min_value=0.5, max_value=t_max_d, step=0.5)
            # Build list of seconds closest to requested minutes
            requested_s = []
            cur = iv_start
            while cur <= iv_end + 1e-6:
                target_s = int(round(cur * 60))
                closest  = min(all_s, key=lambda x: abs(x - target_s))
                if abs(closest - target_s) <= 60 and closest not in requested_s:
                    requested_s.append(closest)
                cur = round(cur + iv_step, 10)
            if requested_s:
                custom_time_s = sorted(set(requested_s))
                st.caption(f"Will plot **{len(custom_time_s)}** time points: "
                           f"{', '.join(str(round(s/60,1)) for s in custom_time_s[:8])}"
                           + (" …" if len(custom_time_s) > 8 else "") + " min")
            else:
                st.warning("No matching time points found for that interval.")

        elif time_mode == "Custom list":
            st.caption("Enter specific time points in **minutes**, comma-separated.  "
                       "Each value will snap to the nearest measured point.")
            custom_txt = st.text_input("Time points (min)",
                                       placeholder="0, 5, 10, 15, 30, 60")
            if custom_txt.strip():
                try:
                    wanted_min = [float(x.strip()) for x in custom_txt.split(",") if x.strip()]
                    snapped_s  = []
                    for wm in wanted_min:
                        target_s = int(round(wm * 60))
                        closest  = min(all_s, key=lambda x: abs(x - target_s))
                        if abs(closest - target_s) <= 120 and closest not in snapped_s:
                            snapped_s.append(closest)
                    if snapped_s:
                        custom_time_s = sorted(set(snapped_s))
                        st.caption(f"Snapped to **{len(custom_time_s)}** point(s): "
                                   f"{', '.join(str(round(s/60,2)) for s in custom_time_s)} min")
                    else:
                        st.warning("None of the entered values matched available time points.")
                except ValueError:
                    st.error("Could not parse — enter numbers separated by commas, e.g. 0, 5, 10")

    # ── Condition selection + line styles ─────────────────────────────────────
    sel_conds = st.multiselect("Conditions to plot", cond_order, default=cond_order)

    LS_OPTIONS = {"Solid (—)": "-", "Dashed (- -)": "--",
                  "Dash-dot (-·-)": "-.", "Dotted (···)": ":"}
    line_styles: dict[str, str] = {}
    if assay_choice == "kinetic" and sel_conds:
        with st.expander("Line styles per condition", expanded=False):
            st.caption("Choose the line style for each condition in the kinetic plot.")
            ls_cols = st.columns(min(len(sel_conds), 3))
            for i, cond in enumerate(sel_conds):
                chosen = ls_cols[i % len(ls_cols)].selectbox(
                    cond, list(LS_OPTIONS.keys()),
                    index=0, key=f"ls_{i}_{cond}"
                )
                line_styles[cond] = LS_OPTIONS[chosen]
    ...
    with st.spinner("Analysing…"):
        try:
            ...
            # Time filter (kinetic)
            if assay_choice == "kinetic":
                if custom_time_s is not None:
                    # Specific points (interval or custom list) override range
                    analysis_df = analysis_df[
                        analysis_df["Time_s"].astype(int).isin(custom_time_s)
                    ].copy()
                elif time_filter_s is not None:
                    t0, t1 = time_filter_s
                    analysis_df = analysis_df[
            ...
            if assay_choice == "endpoint":
                ...
                png = plot_kinetic(summary, active_colors, plot_title, y_label,
                                   line_styles=line_styles if line_styles else None)
...
**Kinetic time-point options:**
- *All time points* — plot everything in the file.
- *Range* — drag a slider to set start and end in minutes.
- *Fixed interval* — enter a start, end, and step (e.g. 0 → 60 min every 5 min). The app snaps each requested minute to the nearest measured point.
- *Custom list* — type specific minutes separated by commas (e.g. `0, 5, 10, 30, 60`). Each value snaps to the nearest measured point.

**Line styles (kinetic only):**  Expand *"Line styles per condition"* to choose Solid, Dashed, Dash-dot, or Dotted for each condition independently.
